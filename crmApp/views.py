...
def productInfoDetail(request, id):
    try:
        ...
        return render(request, context={'product_info': product_detail, 'product_detail_info': productDetailInfo,
                                        'skulist': skulist},
...
def OrderInfoDetail(request, id):
    def list_of_choises():  # делаем списки для выбора ПВЗ в выпадающем меню
        for i in DEPARTURE_CITIES:
            form.fields[i['id']].choices = list(
                map(lambda x: (x['info']['id'], x['info']['name'] + ' - ' + x['result']['price']),
                    orderDetailInfo.cdektarif_tarifes[i['id']]))
        form.fields['selectPVZ'].choices = list(
            map(lambda x: (x['code'], x['location']['address']), orderDetailInfo.cdek_pvz))
